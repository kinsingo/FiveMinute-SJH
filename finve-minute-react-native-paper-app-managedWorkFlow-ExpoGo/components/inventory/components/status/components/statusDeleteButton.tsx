...
export default function StatusDeleteButton({
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setError(
          data.message || "삭제 요청에 실패했습니다. 다시 시도해 주세요."
        );
      }
      ...
      setError(
        "삭제 요청에 실패했습니다. 다시 시도해 주세요." + "error: " + err
      );
    } finally {
      setIsDeleting(false);
    }
  ...
  return (
    <View>
      <Button mode="contained-tonal" style={{marginHorizontal:40}} onPress={handleDelete} disabled={isDeleting}>
